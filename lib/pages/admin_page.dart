...
class _AdminPageState extends State<AdminPage>
  ...
  final _sections = const [
    {'key': 'fashion',    'labelAr': 'الأزياء التقليدية',  'labelFr': 'Mode traditionnelle'},
    {'key': 'handmade',   'labelAr': 'الصناعة اليدوية',    'labelFr': 'Artisanat'},
    {'key': 'traditions', 'labelAr': 'العادات الموريتانية', 'labelFr': 'Traditions'},
  ...
  Widget _buildPreviewImage(ThemeData theme) {
    ...
    if (image.isEmpty) return const Center(child: Icon(Icons.image_outlined, size: 36));
  ...
  Future<void> _submit() async {
    ...
    final message = _editingProductId == null
        ? await widget.onAddProduct(
            ...
            stock: stock,
            category: _categoryController.text.isEmpty ? 'Artisanat' : _categoryController.text,
            categoryAr: _categoryArController.text.isEmpty ? 'حرفة يدوية' : _categoryArController.text,
          )
    ...
    try {
      ...
      if (_hEditingDocId == null) {
        ...
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(isAr ? '✅ تمت الإضافة بنجاح' : '✅ Contenu ajouté avec succès')));
      } else {
        await FirebaseFirestore.instance
            ...
            .update(data);
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(isAr ? '✅ تم التحديث بنجاح' : '✅ Contenu mis à jour avec succès')));
      }
      _resetHeritageForm();
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _deleteHeritage(String docId, bool isAr) async {
    ...
    await FirebaseFirestore.instance
        ...
        .delete();
    if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(isAr ? '✅ تم الحذف' : '✅ Élément supprimé')));
  }
    ...
    return Column(
      children: [
        Container(
          ...
          child: Text(loc.translate('adminDashboard'),
        ...
        TabBar(
          ...
          tabs: [
            Tab(icon: const Icon(Icons.inventory_2_outlined),
                text: isAr ? 'المنتجات' : 'Produits'),
            Tab(icon: const Icon(Icons.auto_stories_outlined),
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          TextField(controller: _titleController,
                              decoration: InputDecoration(labelText: loc.translate('productTitle'))),
                          const SizedBox(height: 8),
                          TextField(controller: _titleArController,
                              decoration: InputDecoration(labelText: '${loc.translate('productTitle')} (عربي)')),
                          const SizedBox(height: 8),
                          ...
                          TextField(controller: _descriptionController,
                              decoration: InputDecoration(labelText: loc.translate('productDescription')),
                              maxLines: 1),
                          const SizedBox(height: 8),
                          TextField(controller: _descriptionArController,
                              decoration: InputDecoration(labelText: '${loc.translate('productDescription')} (عربي)'),
                              maxLines: 1),
                          const SizedBox(height: 8),
                          Row(children: [
                            Expanded(child: TextField(controller: _priceController,
                                decoration: InputDecoration(labelText: loc.translate('productPrice')),
                            ...
                            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                              ...
                              TextField(controller: _discountController,
                                  enabled: _enableDiscount,
                                  decoration: InputDecoration(labelText: '${loc.translate('discount')} %'),
                                  keyboardType: TextInputType.number),
                            ])),
                            const SizedBox(width: 8),
                            Expanded(child: TextField(controller: _stockController,
                  ...
                  Card(
                    child: Padding(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(loc.translate('manageProductsTitle'),
                            ...
                            ...widget.products.map((product) => Card(
                              ...
                              child: ListTile(
                                ...
                                trailing: Wrap(spacing: 6, children: [
                                  IconButton(tooltip: loc.translate('editProduct'),
                                      onPressed: () => _startEdit(product),
                                      icon: const Icon(Icons.edit_outlined)),
                                  IconButton(tooltip: loc.translate('deleteProductButton'),
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          Text(
                            _hEditingDocId == null
                                ? (isAr ? 'إضافة محتوى تراثي' : 'Ajouter un contenu patrimonial')
                                : (isAr ? 'تعديل المحتوى' : 'Modifier le contenu'),
                            style: theme.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w700),
                          ),
                          const SizedBox(height: 12),
                          DropdownButtonFormField<String>(
                            value: _selectedSection,
                            decoration: InputDecoration(labelText: isAr ? 'القسم' : 'Section'),
                          ...
                          const SizedBox(height: 12),
                          TextField(controller: _hTitleController,
                              decoration: InputDecoration(
                                  labelText: isAr ? 'العنوان (Français)' : 'Titre (Français)')),
                          const SizedBox(height: 8),
                          TextField(controller: _hTitleArController,
                              decoration: InputDecoration(
                                  labelText: isAr ? 'العنوان (عربي)' : 'Titre (Arabe — عربي)')),
                          const SizedBox(height: 8),
                          TextField(controller: _hBodyController,
                              decoration: InputDecoration(
                                  labelText: isAr ? 'المحتوى (Français)' : 'Contenu (Français)'),
                              maxLines: 3),
                          const SizedBox(height: 8),
                          TextField(controller: _hBodyArController,
                              decoration: InputDecoration(
                                  labelText: isAr ? 'المحتوى (عربي)' : 'Contenu (Arabe — عربي)'),
                              maxLines: 3),
                          const SizedBox(height: 8),
                          TextField(controller: _hImageUrlController,
                              decoration: InputDecoration(
                                labelText: isAr ? 'رابط الصورة (اختياري)' : 'Lien image (optionnel)',
                                hintText: 'https://...',
                              )),
                          const SizedBox(height: 8),
                          SizedBox(
                            width: double.infinity,
                            child: OutlinedButton.icon(
                              onPressed: _hUploadingImage ? null : _pickHeritageImage,
                              icon: _hUploadingImage
                                  ? const SizedBox(width: 16, height: 16,
                                      child: CircularProgressIndicator(strokeWidth: 2))
                                  : const Icon(Icons.photo_library_outlined),
                              label: Text(_hUploadingImage
                                  ? (isAr ? 'جاري التحميل...' : 'Chargement en cours...')
                                  : (isAr ? 'اختر صورة من الهاتف' : 'Choisir une photo depuis le téléphone')),
                            ),
                          ),
                          if (_hImagePreviewUrl != null) ...[
                            ...
                            ClipRRect(
                              borderRadius: BorderRadius.circular(12),
                              child: Image.network(_hImagePreviewUrl!,
                                  height: 150, width: double.infinity,
                                  fit: BoxFit.cover,
                          ...
                          SizedBox(width: double.infinity,
                            child: FilledButton.icon(
                              onPressed: _hSubmitting ? null : () => _submitHeritage(isAr),
                              icon: const Icon(Icons.save_outlined),
                              label: Text(_hSubmitting ? '...'
                                  : _hEditingDocId == null
                                      ? (isAr ? 'إضافة' : 'Ajouter')
                          ...
                          if (_hEditingDocId != null)
                            Padding(
                              ...
                              child: OutlinedButton(
                                ...
                                child: Text(isAr ? 'إلغاء التعديل' : 'Annuler la modification'),
                              ),
                            ),
                        ],
                      ),
                  ...
                  Card(
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          Text(
                            isAr ? 'المحتوى التراثي المضاف' : 'Contenu patrimonial ajouté',
                            style: theme.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w700)),
                          const SizedBox(height: 10),
                              ...
                              return Column(
                                children: docs.map((doc) {
                                  ...
                                  final section = _sections.firstWhere(
                                      ...
                                      orElse: () => {'labelAr': '', 'labelFr': data['section'] ?? ''});
                                  final bodyText = isAr
                                  ...
                                  return Card(
                                    ...
                                    child: ListTile(
                                      leading: CircleAvatar(
                                        backgroundColor: const Color(0xFF8E4A24).withValues(alpha: 0.12),
                                        child: const Icon(Icons.auto_stories_outlined, color: Color(0xFF8E4A24)),
                                      ),
                                      title: Text(titleText),
                                      subtitle: Text('$sectionLabel — $preview'),
                                      trailing: Wrap(spacing: 6, children: [
                                        IconButton(icon: const Icon(Icons.edit_outlined),
                                            onPressed: () => _startEditHeritage(doc)),
                                        IconButton(icon: const Icon(Icons.delete_outline, color: Colors.red),
                                            onPressed: () => _deleteHeritage(doc.id, isAr)),
                                      ]),
                                    ),
                                  );
                                }).toList(),
